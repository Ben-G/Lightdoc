...
(function(expose) {
 ...
 /**
  ...
  Markdown.toHTML = function toHTML( source , dialect , options ) {
    Markdown.imageRootPath = null;
    var input = this.toHTMLTree( source , dialect , options );

    return this.renderJsonML( input );
  };

  Markdown.toHTMLWithImagePath = function toHTML( source , imageRootPath, dialect , options ) {
    Markdown.imageRootPath = imageRootPath;
    var input = this.toHTMLTree( source , dialect , options );
    ...
    inline: {
      ...
      "![": function image( text ) {
        ...
        if ( m ) {
          ...
          if (Markdown.imageRootPath != null) {
            attrs = { alt: m[1], href: Markdown.imageRootPath+'/'+m[2] || "" };
          } else {
            attrs = { alt: m[1], href: m[2] || "" };
          }
        ...
        if ( m ) {
          ...
          if (Markdown.imageRootPath != null) {
            return [ m[0].length, [ "img_ref", { alt: m[1], ref: Markdown.imageRootPath+'/'+m[2].toLowerCase(), original: m[0] } ] ];
          } else {
...
// Include all our depndencies and;
  ...
  expose.toHTMLWithImagePath = Markdown.toHTMLWithImagePath;
